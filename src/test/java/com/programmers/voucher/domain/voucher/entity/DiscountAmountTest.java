...
class DiscountAmountTest {
    @ParameterizedTest
    @DisplayName("바우처 타입과 상관없이, 할인 값이 양수가 아닌 경우 예외가 발생한다.")
    @MethodSource("invalidDiscountAmountArguments")
    void 할인_값_양수_X(VoucherType voucherType, int amount) {
    ...
    @ValueSource(ints = {105, 2000})
    void 할인율_100_초과(int percent) {
        assertThatThrownBy(() -> new DiscountAmount(VoucherType.PERCENT, percent))
                .isInstanceOf(IllegalArgumentException.class)
