...
class RestCustomerControllerTest {
  ...
  void getCustomerListTest() throws Exception {
    ...
    resultActions.andExpect(status().isOk())
        .andExpect(
            jsonPath("$.data.[0].customerId").value(response.get(0).getCustomerId().toString()))
        .andExpect(
            jsonPath("$.data.[1].customerId").value(response.get(1).getCustomerId().toString()))
        .andExpect(
            jsonPath("$.data.[2].customerId").value(response.get(2).getCustomerId().toString()))
        .andExpect(
            jsonPath("$.data.[3].customerId").value(response.get(3).getCustomerId().toString()))
        .andExpect(
            jsonPath("$.data.[4].customerId").value(response.get(4).getCustomerId().toString()));
  }
