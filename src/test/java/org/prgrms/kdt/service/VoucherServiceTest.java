...
class VoucherServiceTest {
    ...
    @ParameterizedTest
    @MethodSource("createVoucherByValidateVoucherTypeArgument")
    void createVoucherByValidateVoucherType(UUID voucherId, int voucherType, int discountAmount, Class<?> voucherClass) {
        assertAll(
                () -> assertThrows(Exception.class, () -> voucherService.createVoucher(UUID.randomUUID(), 3, 10)),
                () -> assertThat(voucherService.createVoucher(voucherId, voucherType, discountAmount).getClass(), is(voucherClass))
        );
    }

    @DisplayName("유효한 discountAmount로만 생성 할 수 있다.")
    @ParameterizedTest
    @MethodSource("createVoucherByValidateDiscountAmountArguments")
    void createVoucherByValidateDiscountAmount(UUID voucherId, int voucherType, int discountAmount) {

        assertThrows(IllegalArgumentException.class,
                () -> voucherService.createVoucher(voucherId, voucherType, discountAmount));
    }
    ...
    void validateReprovide() {
        ...
        Optional<Voucher> returnVoucher = voucherService.provideVoucherToCustomer(voucher.getVoucherId().toString(),
    ...
    private Stream<Arguments> createVoucherByValidateDiscountAmountArguments() {
        return Stream.of(
                Arguments.of(UUID.randomUUID(), 1, -100),
                Arguments.of(UUID.randomUUID(), 1, 0),
                Arguments.of(UUID.randomUUID(), 2, -100),
                Arguments.of(UUID.randomUUID(), 2, 0),
                Arguments.of(UUID.randomUUID(), 2, 110)
        );
    }

    private Stream<Arguments> createVoucherByValidateVoucherTypeArgument() {
        return Stream.of(
                Arguments.of(UUID.randomUUID(), 1, 100, FixedAmountVoucher.class),
                Arguments.of(UUID.randomUUID(), 2, 100, PercentDiscountVoucher.class)
        );
    }
}