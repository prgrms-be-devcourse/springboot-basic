...
public class VoucherTest {
  ...
  void testConvertNumberToVoucherType() {
    assertThat(VoucherType.fromInteger(1), is(VoucherType.FIXED_AMOUNT));
    assertThat(VoucherType.fromInteger(2), is(VoucherType.PERCENT_DISCOUNT));
  }

  @Test
  @DisplayName("선택지를 벗어나는 경우 에러가 발생한다")
  void testConvertNumberToVoucherTypeException() {
    assertThrows(RuntimeException.class, () -> VoucherType.fromInteger(0));
    assertThrows(RuntimeException.class, () -> VoucherType.fromInteger(3));
  }
  ...
  void testFixedAmountVoucherDiscount() {
    ...
    fixedAmountVoucher = new FixedAmountVoucher(UUID.randomUUID(), 100L);
  ...
  void testVoucherFactory() {
    Voucher voucher = VoucherFactory.createVoucher(1, 1000);
    assertThat(voucher instanceof FixedAmountVoucher, is(true));

    voucher = VoucherFactory.createVoucher(2, 10);
  ...
  void testVoucherFactoryException() {
    assertThrows(RuntimeException.class, () -> VoucherFactory.createVoucher(1, 10_000_000));
    assertThrows(RuntimeException.class, () -> VoucherFactory.createVoucher(1, 0));
    assertThrows(RuntimeException.class, () -> VoucherFactory.createVoucher(2, 101));
    assertThrows(RuntimeException.class, () -> VoucherFactory.createVoucher(2, 0));
  }
}
