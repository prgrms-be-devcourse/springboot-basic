...
class MemberHandlerTest {
    ...
    @AfterEach
    void resetMock() {
        reset(MEMBER_SERVICE_MOCK, REQUEST_MOCK, CONSOLE_MODEL_MOCK);
    }
    ...
    void handleCreateRequest() {
        ...
        verify(CONSOLE_MODEL_MOCK, times(1)).setRedirectLink(CREATE_MEMBER_EMAIL);
    ...
    void handleCreateAmountRequest() {
        ...
        when(REQUEST_MOCK.getConsoleModel()).thenReturn(CONSOLE_MODEL_MOCK);
        when(CONSOLE_MODEL_MOCK.getAttributes("name")).thenReturn("tester");

        var modelAndView = MEMBER_HANDLER.handleRequest(REQUEST_MOCK);

        verify(CONSOLE_MODEL_MOCK, times(1)).setRedirectLink(CREATE_MEMBER_COMPLETE);
        assertThat(modelAndView.getResponseCode(), is(INPUT_AND_REDIRECT));
    }
    ...
    void handleCreateCompleteRequest() {
        ...
        when(REQUEST_MOCK.getConsoleModel()).thenReturn(CONSOLE_MODEL_MOCK);
        when(CONSOLE_MODEL_MOCK.getAttributes("name")).thenReturn("tester");
        when(CONSOLE_MODEL_MOCK.getAttributes("email")).thenReturn("email@prgms.com");
