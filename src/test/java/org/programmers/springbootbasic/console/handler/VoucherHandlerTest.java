...
class VoucherHandlerTest {
    ...
    @AfterEach
    void resetMock() {
        reset(VOUCHER_SERVICE_MOCK, CONSOLE_REQUEST_MOCK, CONSOLE_MODEL_MOCK);
    }
    ...
    void handleCreateRequest() {
        ...
        when(CONSOLE_REQUEST_MOCK.getConsoleModel()).thenReturn(CONSOLE_MODEL_MOCK);

        var modelAndView = VOUCHER_HANDLER.handleRequest(CONSOLE_REQUEST_MOCK);
        verify(CONSOLE_MODEL_MOCK, times(1)).setRedirectLink(CREATE_VOUCHER_AMOUNT);
    ...
    void handleCreateAmountRequest() {
        ...
        when(CONSOLE_REQUEST_MOCK.getConsoleModel()).thenReturn(CONSOLE_MODEL_MOCK);
        when(CONSOLE_MODEL_MOCK.getAttributes("type")).thenReturn("2");

        var modelAndView = VOUCHER_HANDLER.handleRequest(CONSOLE_REQUEST_MOCK);

        verify(CONSOLE_MODEL_MOCK, times(1)).setRedirectLink(CREATE_VOUCHER_COMPLETE);
        assertThat(modelAndView.getResponseCode(), is(INPUT_AND_REDIRECT));
    }
    ...
    void handleCreateCompleteRequest() {
        ...
        when(CONSOLE_REQUEST_MOCK.getConsoleModel()).thenReturn(CONSOLE_MODEL_MOCK);
        when(CONSOLE_MODEL_MOCK.getAttributes("type")).thenReturn("1");
        when(CONSOLE_MODEL_MOCK.getAttributes("amount")).thenReturn("3000");

        var modelAndView = VOUCHER_HANDLER.handleRequest(CONSOLE_REQUEST_MOCK);

        verify(VOUCHER_SERVICE_MOCK, times(1)).createVoucher(3000, FIXED);
        verify(CONSOLE_MODEL_MOCK, times(1)).clear();
