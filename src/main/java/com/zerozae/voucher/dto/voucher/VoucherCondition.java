...
public record VoucherCondition(@Nullable String voucherType,
                               ...
                               @Pattern(regexp = "^\\d{4}-\\d{2}-\\d{2}$", message = "날짜 형식은 yyyy-MM-dd 이어야 합니다.") @Nullable String createdAt,
                               @Pattern(regexp = "^\\d{4}-\\d{2}-\\d{2}$", message = "날짜 형식은 yyyy-MM-dd 이어야 합니다.") @Nullable String endCreatedAt) {

    public VoucherCondition(String voucherType, Long discount, String createdAt , String endCreatedAt) {
