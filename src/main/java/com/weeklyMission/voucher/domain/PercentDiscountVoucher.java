...
public class PercentDiscountVoucher implements Voucher{
    ...
    public PercentDiscountVoucher(UUID voucherId, long amount) {
        ...
        if(amount==0) throw new ArithmeticException("0이 될 수 없음");
        if(amount>MAX_PERCENT_VOUCHER_AMOUNT) throw new IllegalArgumentException("%d보다 작아야 함".formatted(MAX_PERCENT_VOUCHER_AMOUNT));
