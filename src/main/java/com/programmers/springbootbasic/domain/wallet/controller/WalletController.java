...
public class WalletController {
    ...
    public CommonResult<String> addWallet(String email, String voucherId) {
        return execute(walletService::createWallet, WalletRequestDto.builder()
    ...
    public CommonResult<String> deleteWallet(String email, String voucherId) {
        return execute(walletService::deleteWallet, WalletRequestDto.builder()
                .email(Email.from(email).getValue())
                .voucherId(UUID.fromString(voucherId))
                .build());
    }

    private CommonResult<String> execute(Consumer<WalletRequestDto> consumer, WalletRequestDto walletRequestDto) {
        try {
            consumer.accept(walletRequestDto);
        } catch (IllegalArgumentException e) {
            log.warn(e.toString());
