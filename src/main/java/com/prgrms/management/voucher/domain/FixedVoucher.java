...
import static com.prgrms.management.config.ErrorMessageType.OUT_OF_RANGE_FIXED_NUMBER;
...
public class FixedVoucher extends Voucher {
    ...
    private void validateFixedAmount(long amount) {
        if (amount < MIN_FIXED_DISCOUNT || amount > MAX_FIXED_DISCOUNT)
            throw new NumberFormatException(VoucherType.class + OUT_OF_RANGE_FIXED_NUMBER.getMessage());
    }
}
