...
public class Console {
    ...
    public void printDiscountAmountMessage(VoucherType voucherType){
        switch (voucherType){
            case FIXED_AMOUNT -> printer.printMessage(MessageFormat.format(TYPE_FIXED_AMOUNT_MESSAGE, voucherType.getInputValue()));
            case PERCENT -> printer.printMessage(MessageFormat.format(TYPE_PERCENT_MESSAGE, voucherType.getInputValue()));
        }
    }

    public void printAmountOutOfBoundMessage(VoucherType voucherType, String exceptionMessage){
        switch (voucherType){
            case FIXED_AMOUNT -> printer.printMessage(MessageFormat.format(FIXED_AMOUNT_OUT_OF_BOUND, exceptionMessage));
