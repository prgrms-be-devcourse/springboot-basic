...
public class WalletJdbcRepository implements WalletRepository{
    ...
    private VoucherWalletResponseDtos mapVoucherResponse(ResultSet resultSet) throws SQLException {
        return new VoucherWalletResponseDtos(resultSet.getLong("id"), resultSet.getInt("discount"), VoucherDiscountType.valueOf(resultSet.getString("discount_type")), resultSet.getLong("wallet_id"));
    }

    private Customer mapCustomerResponse(ResultSet resultSet) throws SQLException {
    ...
    public List<VoucherWalletResponseDtos> findVouchersByCustomerId(Long id) {
        String findVouchersByCustomerSql = "SELECT V.id as id, V.discount as discount, V.discount_type as discount_type, W.id as wallet_id FROM WALLETS W JOIN Vouchers V ON W.voucher_id = V.id  WHERE W.customer_id = ?";
        return jdbcTemplate.query(findVouchersByCustomerSql, new Object[]{id}, (resultSet, i) -> mapVoucherResponse(resultSet));
    }
    ...
    public List<Customer> findCustomersByVoucherId(Long id) {
        ...
        return jdbcTemplate.query(findCustomersByVoucherSql, new Object[]{id}, (resultSet, i) -> mapCustomerResponse(resultSet));
    }
