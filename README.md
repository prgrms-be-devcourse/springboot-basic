...
- [X] 상수대신 enum 사용하기
- [ ] Repository 클래스의 SQL문을 굳이 상수로 빼야할까 생각하기.
  - 상수 빼면 장점
    - 가독성
    - 성능(불필요한 객체 생성을 막아줌)
    - Repository의 SQL은 상수로 빼는 것 보다 그 자리에 쓰는게 IDE가 예약어를 구분해줘 가독성 측면에서 더 좋다.
    - Repository를 @Bean으로 등록하면 싱글톤이 보장되므로 성능면에서도 큰 이점이 되지 않는다.